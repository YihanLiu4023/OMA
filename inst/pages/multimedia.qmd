...
sample_ids <- colData(se_relative)$SampleID
...
colData(se_relative)$dysbiosis <- sapply(seq_along(ref_set), function(i) {
  ...
  ref_others <- ref_set & (sample_ids != sample_ids[i])
  
  # Compute median distance between sample i and these reference samples
  median(dist[i, ref_others], na.rm = TRUE)
...
treatment <- factor(colData(tse_relative)$Time_point, levels = c("T0", "T1"))
...
M <- assay(tse_relative, "scaled")

# Convert to tibble
mediators <- as_tibble(M)

# Create the mediation data frame
df <- data.frame(
  treatment = treatment,
  dysbiosis = tse_relative$dysbiosis
) %>%
  bind_cols(mediators)
...
the outcome, but pathways abundances as the mediation variables instead 
...
treatment <- factor(colData(tse_pathway)$Time_point, levels = c("T0", "T1"))
...
tse_pathway <- transformAssay(tse_pathway, assay.type="relabundance", method="standardize", name="scaled")
M <- assay(tse_pathway, "scaled")

raw <- colnames(M)
clean1 <- str_remove_all(raw, "\\[|\\]")
clean2 <- str_replace_all(clean1, "[: \\.,]", "_")
safe_names <- make.names(clean2, unique = TRUE)
colnames(M) <- safe_names

mediators <- as_tibble(M)

df <- data.frame(
  treatment = treatment,
  dysbiosis = tse_pathway$dysbiosis
) %>%
  bind_cols(mediators)
